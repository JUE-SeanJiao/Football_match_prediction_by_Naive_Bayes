...
with open('Match.csv','r') as f:
    ...
    for line in f:
        mat = line.rstrip('\n').split(',')
        matID = mat[1]
        if matID in possMat.keys():
            tmp = mat[4].split(' ')
            dt = tmp[0]
            dom_matches.append([mat[3],parse(dt),mat[5],mat[6],mat[7],mat[8],country2id[mat[2]],possMat[matID][0],possMat[matID][1]])
